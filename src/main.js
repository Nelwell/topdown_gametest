(function () {
  ...
  window.addEventListener('pointerdown', requestAudioUnlock, { once: true });
  window.addEventListener('keydown', requestAudioUnlock, { once: true });
  ...
  function playSound(name) {
    ...
    switch (name) {
      case 'step': {
        ...
        tone.frequency.exponentialRampToValueAtTime(70, now + 0.25);
        const toneGain = ctx.createGain();
        toneGain.gain.setValueAtTime(0, now);
        toneGain.gain.linearRampToValueAtTime(0.45, now + 0.02);
        ...
        noiseGain.gain.linearRampToValueAtTime(0.32, now + 0.02);
      ...
      case 'swing': {
        ...
        swish.buffer = getNoiseBuffer(ctx, 0.3);
        ...
        swishGain.gain.linearRampToValueAtTime(0.65, now + 0.05);
        swishGain.gain.exponentialRampToValueAtTime(0.01, now + 0.28);
        band.connect(swishGain);
        swishGain.connect(audioMasterGain);
        swish.start(now);
        swish.stop(now + 0.32);
        ...
        const toneGain = ctx.createGain();
        toneGain.gain.setValueAtTime(0, now);
        toneGain.gain.linearRampToValueAtTime(0.3, now + 0.02);
      ...
      case 'dash': {
        const dash = ctx.createOscillator();
        dash.type = 'square';
        dash.frequency.setValueAtTime(220, now);
        dash.frequency.exponentialRampToValueAtTime(620, now + 0.12);
        const dashGain = ctx.createGain();
        dashGain.gain.setValueAtTime(0, now);
        dashGain.gain.linearRampToValueAtTime(0.4, now + 0.03);
        dashGain.gain.exponentialRampToValueAtTime(0.002, now + 0.42);
        dash.connect(dashGain);
        dashGain.connect(audioMasterGain);
        dash.start(now);
        dash.stop(now + 0.45);
        break;
      }
      case 'hurt': {
        const hurt = ctx.createOscillator();
        hurt.type = 'triangle';
        hurt.frequency.setValueAtTime(360, now);
        hurt.frequency.exponentialRampToValueAtTime(140, now + 0.45);
        ...
        vibratoGain.connect(hurt.frequency);
        vibrato.start(now);
        vibrato.stop(now + 0.45);
        const hurtGain = ctx.createGain();
        hurtGain.gain.setValueAtTime(0, now);
        hurtGain.gain.linearRampToValueAtTime(0.35, now + 0.03);
        hurtGain.gain.exponentialRampToValueAtTime(0.002, now + 0.5);
        hurt.connect(hurtGain);
        hurtGain.connect(audioMasterGain);
        hurt.start(now);
        hurt.stop(now + 0.5);
        break;
      }
  ...
  function updatePlayer(dt) {
    ...
    if (isMoving && player.dashTime <= 0) {
      player.stepTimer += dt;
      const interval = player.stepInterval;
      while (player.stepTimer >= interval) {
  ...
  function drawPlayer() {
    ...
    const spriteOffsetX = -spriteWidth / 2;
    const spriteOffsetY = 6 - spriteHeight;
    const swingProgress =
      player.attackTimer > 0 && player.swordDuration > 0
        ? clamp(1 - player.attackTimer / player.swordDuration, 0, 1)
        : 0;
    ...
    const swordBehind = player.facing === 'up' && swingProgress < 0.55;
    if (swordBehind) {
      drawPlayerSword(player, spriteOffsetX, spriteOffsetY, swingProgress);
    }

    drawPlayerSpriteBody(spriteOffsetX, spriteOffsetY);
    ...
    if (player.hitFlash > 0) {
      const flashAlpha = clamp(player.hitFlash / 0.4, 0, 1);
      ctx.fillStyle = `rgba(255, 255, 255, ${0.55 * flashAlpha})`;
      ctx.fillRect(spriteOffsetX - 1, spriteOffsetY - 2, spriteWidth + 2, spriteHeight + 4);
    }

    ctx.restore();
  }

  function drawPlayerSpriteBody(offsetX, offsetY) {
    const paint = (x, y, w, h, color) => {
      ctx.fillStyle = color;
      ctx.fillRect(offsetX + x, offsetY + y, w, h);
    };

    paint(3, 16, 4, 2, '#2b1d15');
    paint(9, 16, 4, 2, '#2b1d15');
    paint(3, 14, 4, 2, '#5b3d2f');
    paint(9, 14, 4, 2, '#5b3d2f');
    paint(3, 14, 1, 2, '#3a261b');
    paint(12, 14, 1, 2, '#3a261b');
    paint(4, 13, 3, 1, '#2d211a');
    paint(9, 13, 3, 1, '#2d211a');
    paint(4, 12, 3, 2, '#2a6b30');
    paint(9, 12, 3, 2, '#2a6b30');
    paint(4, 12, 1, 2, '#1f5128');
    paint(11, 12, 1, 2, '#1f5128');
    paint(5, 12, 1, 2, '#3f8f4a');
    paint(10, 12, 1, 2, '#3f8f4a');

    paint(3, 6, 10, 6, '#2f8f3f');
    paint(3, 6, 1, 6, '#1f6c2f');
    paint(12, 6, 1, 6, '#1f6c2f');
    paint(2, 6, 1, 6, '#194921');
    paint(13, 6, 1, 6, '#194921');
    paint(4, 7, 8, 5, '#3fb14f');
    paint(5, 8, 6, 4, '#4ac869');
    paint(5, 11, 6, 1, '#1f5a2b');
    paint(4, 11, 1, 1, '#1f5a2b');
    paint(11, 11, 1, 1, '#1f5a2b');
    paint(5, 7, 1, 4, '#27823a');
    paint(10, 7, 1, 4, '#27823a');

    paint(3, 10, 10, 1, '#b8842c');
    paint(4, 9, 8, 1, '#e2c067');
    paint(6, 9, 1, 1, '#c58b35');
    paint(9, 9, 1, 1, '#c58b35');
    paint(7, 9, 2, 2, '#f0cf6b');
    paint(7, 10, 2, 1, '#d7a84f');
    paint(3, 11, 10, 1, '#1b4b25');
    paint(4, 12, 8, 1, '#2b7038');

    paint(1, 7, 2, 5, '#2f8f3f');
    paint(13, 7, 2, 5, '#2f8f3f');
    paint(1, 7, 1, 5, '#1f6c2f');
    paint(14, 7, 1, 5, '#1f6c2f');
    paint(2, 8, 1, 2, '#3fb14f');
    paint(13, 8, 1, 2, '#3fb14f');
    paint(1, 11, 2, 1, '#18491f');
    paint(13, 11, 2, 1, '#18491f');
    paint(1, 12, 2, 2, '#d8b48a');
    paint(13, 12, 2, 2, '#d8b48a');
    paint(1, 12, 2, 1, '#e7c9a4');
    paint(13, 12, 2, 1, '#e7c9a4');

    paint(6, 7, 1, 4, '#ba842f');
    paint(7, 7, 1, 4, '#e4bf5a');
    paint(8, 7, 1, 4, '#ba842f');
    paint(6, 11, 3, 1, '#8c6327');

    paint(5, 6, 6, 1, '#1f6c2f');
    paint(6, 6, 4, 1, '#3fb14f');

    paint(5, 2, 6, 5, '#f6d2a1');
    paint(6, 3, 4, 1, '#fbe5c4');
    paint(6, 6, 4, 1, '#e0b98b');
    paint(6, 4, 1, 1, '#1f1f1f');
    paint(9, 4, 1, 1, '#1f1f1f');
    paint(7, 5, 2, 1, '#d79a61');
    paint(7, 6, 2, 1, '#b67335');
    paint(4, 3, 2, 4, '#f06aa3');
    paint(10, 3, 2, 4, '#f06aa3');
    paint(5, 1, 6, 2, '#f06aa3');
    paint(5, 1, 6, 1, '#d94b83');
    paint(4, 2, 2, 1, '#d94b83');
    paint(10, 2, 2, 1, '#d94b83');
    paint(4, 5, 1, 2, '#c4427a');
    paint(11, 5, 1, 2, '#c4427a');

    paint(3, 4, 2, 3, '#f6d2a1');
    paint(11, 4, 2, 3, '#f6d2a1');
    paint(3, 5, 2, 1, '#e0b98b');
    paint(11, 5, 2, 1, '#e0b98b');

    paint(2, 0, 12, 3, '#2c9434');
    paint(3, 1, 10, 1, '#3fb14f');
    paint(5, 1, 4, 1, '#54c968');
    paint(3, 2, 10, 1, '#1f6c2f');
    paint(2, 0, 1, 3, '#1d5d2a');
    paint(13, 0, 1, 3, '#1d5d2a');
    paint(2, 2, 2, 1, '#1f6c2f');
    paint(2, 3, 2, 1, '#3fb14f');
    paint(12, 2, 3, 4, '#1b6b2a');
    paint(12, 3, 2, 1, '#3fb14f');
    paint(13, 4, 2, 1, '#1f6c2f');
    paint(12, 5, 2, 1, '#3fb14f');
    paint(14, 5, 1, 1, '#54c968');

    ctx.fillStyle = '#1b6b2a';
    ctx.beginPath();
    ctx.moveTo(offsetX + 14, offsetY + 4);
    ctx.lineTo(offsetX + 16, offsetY + 7);
    ctx.lineTo(offsetX + 12, offsetY + 7);
    ctx.closePath();
    ctx.fill();

    ctx.fillStyle = '#3fb14f';
    ctx.beginPath();
    ctx.moveTo(offsetX + 14, offsetY + 4);
    ctx.lineTo(offsetX + 15, offsetY + 6);
    ctx.lineTo(offsetX + 12, offsetY + 6);
    ctx.closePath();
    ctx.fill();
  }

  function drawPlayerSword(player, offsetX, offsetY, swingProgress) {
    const facingAngles = {
      up: -Math.PI / 2,
      down: Math.PI / 2,
      left: Math.PI,
    ...
    const anchorByFacing = {
      up: { x: offsetX + 8, y: offsetY + 8 },
      down: { x: offsetX + 8, y: offsetY + 12 },
      left: { x: offsetX + 4, y: offsetY + 11 },
      right: { x: offsetX + 12, y: offsetY + 11 }
    };
    const ratioByFacing = {
      up: { start: 0.25, end: 0.88 },
      down: { start: 0.7, end: 0.2 },
      left: { start: 0.88, end: 0.18 },
      right: { start: 0.12, end: 0.9 }
    };
    const facing = facingAngles[player.facing] != null ? player.facing : 'down';
    const baseAngle = facingAngles[facing];
    const swingArc = Math.PI * 0.95;
    const swingStart = baseAngle - swingArc / 2;
    const swingEnd = baseAngle + swingArc / 2;
    const settings = ratioByFacing[facing] || ratioByFacing.down;
    const eased = swingProgress > 0 ? easeOutCubic(swingProgress) : 0;
    const ratio = settings.start + (settings.end - settings.start) * eased;
    const anchor = anchorByFacing[facing] || anchorByFacing.down;

    ctx.save();
    ctx.translate(Math.round(anchor.x), Math.round(anchor.y));
    ctx.rotate(swingStart + (swingEnd - swingStart) * ratio);
    ctx.fillStyle = '#2b1d15';
    ctx.fillRect(-4, -2, 4, 4);
    ctx.fillStyle = '#c2a046';
    ctx.fillRect(-5, -1, 1, 2);
    ctx.fillStyle = '#d4b04a';
    ctx.fillRect(0, -3, 3, 6);
    ctx.fillStyle = '#b38934';
    ctx.fillRect(-1, -4, 5, 1);
    ctx.fillRect(-1, 3, 5, 1);
    ctx.fillStyle = '#e5f4ff';
    ctx.fillRect(3, -2, 18, 4);
    ctx.fillStyle = '#b7d8ff';
    ctx.fillRect(3, -1, 18, 2);
    ctx.fillStyle = '#f5fbff';
    ctx.fillRect(11, -2, 2, 4);
    ctx.fillStyle = '#90b7ff';
